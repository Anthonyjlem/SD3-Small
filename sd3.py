...
class CustomModulatedLayerNorm(nn.Module):
    ...
    def forward(self, x, a_x, b_x):
        ...
        # a_x: B x embed
        x = nn.LayerNorm(x, elementwise_affine=False)
...
class CustomMLP(nn.Module):
    ...
    def forward(self, xc, x, g, d, e, z):
        ...
        x_1 = x_1 * g[:,None,:]
        x_1 = x_1 + x
        x_1 = nn.LayerNorm(x_1, elementwise_affine=False)
...
class MMDiTBlock(nn.Module):
    ...
    def forward(self, x, c, y):
        ...
        xc = self._atten(q, k, v)  # B x (num_patches + 77) x head_dim*n_heads
...
class DiT(nn.Module):
    ...
    def forward(self, latent, ctx, ctx_pool, t):
        ...
        divisor = self._unpatching(self._patching(torch.ones(latent.shape, dtype=latent.dtype)))
    ...
    def get_vae_latent(self, image):
        with torch.no_grad():
            latent = self._vae.encode(image).latent_dist.mean
